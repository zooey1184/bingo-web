				...
				$items.each(function(index_id){
					...
					if(global_online == 'true'){
					  	if(global_ip_valid == 'true' || global_sn_valid == true){
					   		if(finished == 'false'){
					      		divHTML += "<div class='link_out'><a href='" + url + "&testService=cet6"  +"' target='_blank'><img src='/st/images/gaokao_1/tools/begin.gif' border=0/></a></div>";
					        }else{			
	...
	/*
		...
		*/
		...
		var $topicTbody = $('#tbody_topic_'+ topic_id);
			...
			$(document).ready(function(){
		   $.get('/lib/vocbook/getvoccount.xml?topic=' + topic_id + '&band=6', function(data){
         ...
         var tb_3_html = "<td><div id='div_vocbooklink'>" + vocCount +"</div></td>";
		      ...
		      if(global_online == 'true'){
		        $('#vocMenu').find('td:eq(2)').click(function(){
		          window.open("/lib/vocbook/index.do?topic="+topic_id+"&band=6&org=cet6","_self");
		                     });
		                }
...
function init_buy() {
	...
	strHTML +="<div class='alertmsg'><table><tr><td><img src='/st/images/gaokao_1/account/forbid.png' border='0'></td><td align='right'>您还没有购买该产品<br />或者您的服务已经到期，请先购买<br/><hr class='dashedhr'/><a href='/lib/account/cet6/index.do' target='_self'><img src='/st/images/buy.gif' border='0'/></a></td></tr></table><div>";
