...
function loaddefaultcontent( topic_id, ability ){
		...
		$.get('/lib/mybingo/getNewAbilityMaterialsList.xml?testService=zju3&ability=' + ability+'&topic='+topic_id + '&bingoBand=2&ran=' + ran, function(data){
		$('#materials').empty();
		$('#materials').unblock();
				...
				$items.each(function(index_id){
					...
					if(global_online == 'true'){
						if(global_ip_valid == 'true' || global_sn_valid == true){
					    		if(finished == 'false'){
					      			divHTML += "<div class='link_out'><a href='" + url + "&testService=zju3"  +"' target='_blank'><img src='/st/images/gaokao_1/tools/begin.gif' border=0/></a></div>";
					       		}else{			
	...
	/*
		...
		*/
		...
		$(document).ready(function(){
		   $.get('/lib/vocbook/getvoccount.xml?topic=' + topic_id + '&band=3', function(data){
		      ...
		      if(global_online == 'true'){
		        $('#vocMenu').find('td:eq(2)').click(function(){
		          window.open("/lib/vocbook/index.do?topic="+topic_id+"&band=3&org=zj3","_self");
		                     });
		                }
...
function init_buy() {
	...
	strHTML +="<div class='alertmsg'><table><tr><td><img src='/st/images/gaokao_1/account/forbid.png' border='0'></td><td align='right'>您还没有购买该产品或者您的服务已经到期，请先购买<br/><hr class='dashedhr'/><a href='/lib/account/zju3/index.do' target='_self'>购买</a></td></tr></table><div>";
