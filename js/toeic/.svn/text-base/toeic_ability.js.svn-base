...
function loaddefaultcontent( topic_id, ability ){
		...
		$.get('/lib/mybingo/getNewAbilityMaterialsList.xml?testService=toeic&ability='+ability+'&topic='+topic_id + '&bingoBand=4&ran=' + ran, function(data){
		$('#materials').empty();
		$('#materials').unblock();
	 ...
	 global_online = $(data).find('online').text();
		...
		$(data).find('todo').each( function(){
			...
			if( item_length > 0 ) {
				...
				$items.each(function(index_id){
					...
					divHTML += "<div class='link_out'><a href='" + url  + "&testService=toeic" +"' target='_blank'><img src='/st/images/gaokao_1/tools/begin.gif' border=0/></a></div>";
					}
		...
		$(document).ready(function(){
		   $.get('/lib/vocbook/getvoccount.xml?topic=' + topic_id + '&bingoBand=4', function(data){
		      ...
		      if(global_online == 'true'){
		        $('#vocMenu').find('td:eq(2)').click(function(){
		          window.open("/lib/vocbook/index.do?topic="+topic_id+"&bingoBand=4&org=toeic","_self");
		                     });
		                }
		      else if(global_online == 'false'){
		        $('#vocMenu').find('td:eq(2)').click(function(){
...
function init_buy() {
	...
	strHTML +="<div class='alertmsg'><table><tr><td><img src='/st/images/gaokao_1/account/forbid.png' border='0'></td><td align='right'>您还没有购买该产品或者您的服务已经到期，请先购买<br/><hr class='dashedhr'/><a href='/lib/account/toeic/index.do' target='_self'>购买</a></td></tr></table><div>";
