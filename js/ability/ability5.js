...
function loaddefaultcontent( topic_id, ability ){
		...
		$.get('/lib/mybingo/getNewAbilityMaterialsList.xml?testService=ability&ability='+ability+'&topic='+topic_id + '&bingoBand=5&ran=' + ran, function(data){
		$('#materials').empty();
		$('#materials').unblock();
	...
	/*
		$(data).find('history').each( function(){
			...
			if( item_length > 0 ) {
				var divHTML = "<div class='history_item'>已完成题目:<br />";
		...
		*/
		...
		$(document).ready(function(){
		   $.get('/lib/vocbook/getvoccount.xml?topic=' + topic_id + '&bingoBand=5', function(data){
         ...
         var tb_3_html = "<td><div id='div_vocbooklink'>" + vocCount +"</div></td>";
		      ...
		      if(global_online == 'true'){
		        $('#vocMenu').find('td:eq(2)').click(function(){
		          window.open("/lib/vocbook/index.do?topic="+topic_id+"&band=15&org=ability5","_self");
		                     });
		                }
