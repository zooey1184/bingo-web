...
function loaddefaultcontent( topic_id, ability ){
		...
		$.get('/lib/mybingo/getNewAbilityMaterialsList.xml?testService=gaokao&ability='+ability+'&topic='+topic_id + '&bingoBand=2&ran=' + ran, function(data){
		$('#materials').empty();
		$('#materials').unblock();
	...
	/*
		$(data).find('history').each( function(){
			...
			if( item_length > 0 ) {
				...
				$items.each(function(){
					...
					divHTML += "<div class='link_out'>"
					divHTML += "得分：" + score +"<br />";
					divHTML +="<a href='" + url + "&testService=zju3" +"' target='_blank'><img src='/st/images/gaokao_1/tools/redo.gif' border=0/></a><br/>";
					divHTML += "<a href='" + redo_url +"' target='_blank'><img src='/st/images/gaokao_1/tools/review.gif' border=0/></a></div>";
					divHTML += "<div class='cleared'></div>";
					divHTML += "</div>";
				});
		...
		*/
		...
		$(document).ready(function(){
		   $.get('/lib/vocbook/getvoccount.xml?topic=' + topic_id + '&band=22', function(data){
			...
			var tb_4_html = "<td valign='top'><div id='div_choose_level'><a href='#null' onclick='block_with_choose()'><img src='/st/images/ability/level_2.gif' border=0/></a></div></td>";
		      ...
		      if(global_online == 'true'){
		        $('#vocMenu').find('td:eq(2)').click(function(){
		          window.open("/lib/vocbook/index.do?topic="+topic_id+"&band=22&org=ability2","_self");
		                     });
		                }
